...
if [[ "$USE_GOSEC_SSO_AUTH" != "true" ]]; then
    ...
    NGINX_ERROR_LOG_LEVEL=$NGINX_ERROR_LOG_LEVEL envsubst '\$NGINX_ERROR_LOG_LEVEL \$MARATHON_APP_ID' < ${nginx_conf_dir}/nginx.conf.native-auth > ${nginx_conf_dir}/nginx.conf
else
    # we are delegating authentication in dcos-oauth so we use the corresponding nginx.conf
    NGINX_ERROR_LOG_LEVEL=$NGINX_ERROR_LOG_LEVEL envsubst '\$NGINX_ERROR_LOG_LEVEL \$MARATHON_APP_ID' < ${nginx_conf_dir}/nginx.conf.sso-auth > ${nginx_conf_dir}/nginx.conf
fi
