...
#    When I send a 'POST' request to '/${DISCOVERY_ID:-discovery-qa}/api/database' based on 'schemas/registerdatabase.json' as 'json' with:
      ...
      | $.details.host                                  | UPDATE  | pg-0001.${DCOS_TENANT1}-${POSTGRES_NAME:-postgrestls}.${DCOS_TENANT1}.mesos                                                                                                                                                                     | string |
      ...
      | $.details.user                                  | UPDATE  | ${DCOS_TENANT1}-${DISCOVERY_ID:-discovery-qa}                                                                                                                                                   | string |
      | $.details.additional-options                    | UPDATE  | ssl=true&sslmode=verify-full&sslcert=/root/kms/${DCOS_TENANT1}-${DISCOVERY_ID:-discovery-qa}.pem&sslkey=/root/kms/${DCOS_TENANT1}-${DISCOVERY_ID:-discovery-qa}.pk8&sslrootcert=/root/kms/root.pem      | string |
